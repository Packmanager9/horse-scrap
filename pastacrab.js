...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Node{
        constructor(knee = 0, foot = 0){

            this.foot = new Circle(350, 400, 4, "cyan")
            if(knee == 0){
                this.knee = new Circle(350, 350, 2, "red")
            }else{

                this.foot.y+=(nodes.length+1)
                this.knee = knee
            }
            if(foot == 0){
                this.foot = new Circle(350, 400, 4, "cyan")
            }else{
                this.knee.y-=(nodes.length+1)
                this.foot = foot
            }
            this.femur = new LineOP(this.foot, this.knee, "white", 1)
            this.gravity = .0001
            this.length = 50
        }
        draw(){
            // this.foot.ymom += this.gravity
            // this.knee.ymom += this.gravity
            this.foot.move()
            this.knee.move()
            this.femur.draw()
            this.foot.draw()
            this.knee.draw()
            let vec = new Vector(this.knee, (this.knee.x-this.foot.x),( this.knee.y-this.foot.y))
            vec.normalize(this.length)
            if(this.knee.flagged != 1){
                this.knee.x = this.foot.x + vec.xmom
                this.knee.y = this.foot.y + vec.ymom
            }else{

            // if(this.foot.flagged != 1){
                this.foot.x = this.knee.x - vec.xmom
                this.foot.y = this.knee.y - vec.ymom
            // }
            }
        ...
        apply(vec){
            this.knee.x+=(vec.xmom/2)
            this.knee.y+=(vec.ymom/2)
            this.foot.x+=(vec.xmom/2)
            this.foot.y+=(vec.ymom/2)
        }
    }
    ...
    class MetaNode{
        constructor(knee = 0, foot = 0){
            ...
            this.foot.flagged = 1
            this.knee.flagged = 1
            this.femur = new SpringOP(this.foot, this.knee, this.length, 3)
            this.gravity = .0001
        }
        draw(){
            // this.knee.radius = 10
            this.foot.radius = 10
            this.foot.color = getRandomColor()
            this.knee.color = getRandomColor()
            // this.knee.radius = 10
            // this.foot.ymom += this.gravity
            // this.knee.ymom += this.gravity
            this.foot.move()
    ...
    let node2 = new Node(0, node.knee)
    let node3 = new Node(0, node2.knee)
    node3.knee.y-=50
    ...
    let vec = new Vector(node.knee, 1, .1)


    // body.foot.x = 300


    let nodex = new Node(0,0)
    let node2x = new Node(0, nodex.knee)
    let node3x = new Node(0, node2x.knee)
    ...
    let body = new MetaNode(node3.knee, node3x.knee)
    ...
    class Parnod {
        constructor(x,y, parent = this){
            this.parent = parent
            this.body = new Circle(x,y, 3, "red")
            this.children = []
            this.lengths = []
            this.gravity = .6
            this.ymom = 0
        ...
        append(child){
            ...
            this.lengths.push((new LineOP(this.body, child.body, "red", 1).hypotenuse()*.99))
        }
        applyShift(vec){
            ...
            for(let t = 0;t<this.children.length;t++){
                this.children[t].applyShift(vec)
            }
        }
        backStep(vec){
            this.ymom = 0
            this.parent.body.x+=vec.xmom
            this.parent.body.y+=vec.ymom
            if(this != this.parent){
                this.parent.backStep(vec)
            }
        }
        centralize(){
            // let point = new Point((this.body.x +this.children[0].body.x)*.5, (this.body.y +this.children[0].body.y)*.5)

            // let vec1 = new Vector(this.body, this.body.x-this.children[0].body.x,  this.body.y-this.children[0].body.y)
            // let vec2 = new Vector(this.children[0].body, this.children[0].body.x-this.body.x,  this.children[0].body.y-this.body.y)
            // vec1.normalize(this.lengths[0])
            // vec2.normalize(this.lengths[0])

            // this.body.x = point.x + (vec1.xmom*.5)
            // this.body.y = point.y + (vec1.ymom*.5)
            // this.children[0].body.x = point.x + (vec2.xmom*.5)
            // this.children[0].body.y = point.y + (vec2.ymom*.5)



            for(let t = 0;t<this.children.length;t++){

                let point = new Point((this.body.x +this.children[t].body.x)*.5, (this.body.y +this.children[t].body.y)*.5)

                let vec1 = new Vector(this.body, this.body.x-this.children[t].body.x,  this.body.y-this.children[t].body.y)
                let vec2 = new Vector(this.children[t].body, this.children[t].body.x-this.body.x,  this.children[t].body.y-this.body.y)
                vec1.normalize(this.lengths[t])
                vec2.normalize(this.lengths[t])
    
                this.body.x = point.x + (vec1.xmom*.5)
                this.body.y = point.y + (vec1.ymom*.5)
                this.children[t].body.x = point.x + (vec2.xmom*.5)
                this.children[t].body.y = point.y + (vec2.ymom*.5)
            }
        ...
        draw(){
            this.body.draw()
            for(let t = 0;t<this.children.length;t++){
                let link = new LineOP(this.body, this.children[t].body, "#00FF00", 1)
                link.draw()
            }
        }
        live(){
            let storage = {}
            storage.x = this.body.x
            storage.y = this.body.y
            this.ymom += this.gravity
            this.body.y += this.ymom
            if(this.body.doesPerimeterTouch(floor)){
                let vec = new Vector(this.body, this.body.x-floor.x, this.body.y-floor.y)
                vec.normalize(floor.radius+1)
                this.body.x = floor.x + vec.xmom
                this.body.y = floor.y + vec.ymom
            }
            let vec = new Vector(this.body, (this.body.x-storage.x)*30, (this.body.y-storage.y))
            this.applyShift(vec)
    ...
    let nod1 = new Parnod(350,400)
    let nod2 = new Parnod(300,300, nod1)
    nod1.append(nod2)
    let nod3 = new Parnod(250,350, nod2)
    nod2.append(nod3)
    let nod4 = new Parnod(200,350, nod2)
    nod2.append(nod4)
    let nod5 = new Parnod(300,350, nod4)
    nod4.append(nod5)


    let floor = new Circle(400,100400, 100000, "white")
    let angle  = 0
    let counter = 0
    ...
    let vecs = new Vector({}, Math.cos(angle),Math.sin(angle)*1.5)
    nod5.draw()
    nod4.draw()
    nod3.draw()
    nod2.draw()
